import tkinter as tk
from tkinter import filedialog, scrolledtext, messagebox, ttk
import os
import shutil
import subprocess
import re
from datetime import datetime
import platform
import webbrowser


class FileUploadTool:
    """
    文件上传工具类，提供可视化界面用于管理配置文件、文件操作和执行Go程序。
    """

    def __init__(self, root_window):
        """
        初始化应用程序界面和配置。

        Args:
            root_window: Tkinter根窗口对象
        """
        self.root = root_window
        self.root.title("文件上传工具")
        self.root.geometry("1000x600")  # 加宽窗口以容纳右侧文件浏览面板
        self.root.resizable(True, True)

        # 设置中文字体
        self.font = ("SimHei", 10)
        self.title_font = ("SimHei", 12, "bold")

        # 配置文件路径
        self.conf_file_path = r"D:\project\demo-uploadFile\demo-uploadFile\conf\conf.go"
        self.imgs_dir = r"D:\project\demo-uploadFile\demo-uploadFile\imgs"
        self.imgs_old_dir = r"D:\project\demo-uploadFile\demo-uploadFile\imgsOld"
        self.doc_files_dir = r"D:\project\demo-uploadFile\demo-uploadFile\docFiles"
        self.extract_file_path = r"D:\project\demo-uploadFile\demo-uploadFile\extractFile.go"
        # 新增：上传文件go文件路径
        self.upload_file_path = r"D:\project\demo-uploadFile\demo-uploadFile\uploadFile.go"

        # 新增：对齐GoLand的Go环境配置（从你提供的命令中提取）
        self.goroot = r"D:\soft\go\go"
        self.gopath = r"D:\project\goProject\pkg"
        self.go_bin_path = os.path.join(self.goroot, "bin", "go.exe")

        # ========== 新增：主界面布局调整（左侧标签页 + 右侧文件浏览） ==========
        # 创建主容器
        main_container = ttk.Frame(root_window, padding="10")
        main_container.pack(fill=tk.BOTH, expand=True)

        # 左侧标签页区域（占70%宽度）
        left_frame = ttk.Frame(main_container)
        left_frame.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=(0, 10))

        # 右侧文件浏览区域（占30%宽度）
        right_frame = ttk.Frame(main_container, width=300)
        right_frame.pack(side=tk.RIGHT, fill=tk.BOTH, expand=False)
        right_frame.pack_propagate(False)  # 固定宽度

        # 创建主框架（移到左侧框架）
        self.main_frame = ttk.Frame(left_frame, padding="0")
        self.main_frame.pack(fill=tk.BOTH, expand=True)

        # ========== 原有标签页代码 ==========
        # 创建标签页
        self.tab_control = ttk.Notebook(self.main_frame)
        self.tab_control.pack(fill=tk.BOTH, expand=True)

        # 创建配置标签页
        self.config_tab = ttk.Frame(self.tab_control)
        self.tab_control.add(self.config_tab, text="配置")

        # 创建文件管理标签页
        self.file_tab = ttk.Frame(self.tab_control)
        self.tab_control.add(self.file_tab, text="文件管理")

        # 创建操作标签页
        self.action_tab = ttk.Frame(self.tab_control)
        self.tab_control.add(self.action_tab, text="操作")

        # 创建日志标签页
        self.log_tab = ttk.Frame(self.tab_control)
        self.tab_control.add(self.log_tab, text="日志")

        # 初始化各标签页
        self.init_config_tab()
        self.init_file_tab()
        self.init_action_tab()
        self.init_log_tab()

        # ========== 新增：初始化右侧文件浏览面板 ==========
        self.init_file_browser_panel(right_frame)

        # 初始化日志
        self.logs = []
        self.add_log("应用程序已启动")
        self.add_log(f"Go环境配置 - GOROOT: {self.goroot}, GOPATH: {self.gopath}")

        # 检查目录是否存在
        self.check_directories()

        # 初始加载文件列表
        self.refresh_file_list()

    def init_config_tab(self):
        """初始化配置标签页，包含Cookie配置和路径显示"""
        # Cookie配置区域
        config_frame = ttk.LabelFrame(self.config_tab, text="Cookie配置", padding="10")
        config_frame.pack(fill=tk.X, pady=10, padx=10)

        # Cookie标签
        ttk.Label(config_frame, text="Cookie:", font=self.font).grid(row=0, column=0, sticky=tk.W, pady=5)

        # Cookie输入框（多行，适配超长Cookie）
        self.cookie_text = tk.Text(config_frame, width=80, height=8, font=self.font)
        self.cookie_text.grid(row=1, column=0, columnspan=2, sticky=tk.W + tk.E, pady=5)

        # 加载当前Cookie按钮
        load_cookie_btn = ttk.Button(config_frame, text="加载当前Cookie", command=self.load_current_cookie)
        load_cookie_btn.grid(row=0, column=1, sticky=tk.E, pady=5, padx=5)

        # 保存Cookie按钮
        save_cookie_btn = ttk.Button(config_frame, text="保存Cookie", command=self.save_cookie)
        save_cookie_btn.grid(row=2, column=0, sticky=tk.W, pady=10)

        # 配置文件路径显示
        path_frame = ttk.LabelFrame(self.config_tab, text="配置文件路径", padding="10")
        path_frame.pack(fill=tk.X, pady=10, padx=10)

        ttk.Label(path_frame, text="配置文件:", font=self.font).grid(row=0, column=0, sticky=tk.W, pady=5)
        ttk.Label(path_frame, text=self.conf_file_path, font=self.font).grid(row=0, column=1, sticky=tk.W, pady=5)

        # 目录路径显示
        dir_frame = ttk.LabelFrame(self.config_tab, text="目录路径", padding="10")
        dir_frame.pack(fill=tk.X, pady=10, padx=10)

        # 显示各目录路径
        self._display_directory_path(dir_frame, "图片目录:", self.imgs_dir, 0)
        self._display_directory_path(dir_frame, "图片备份目录:", self.imgs_old_dir, 1)
        self._display_directory_path(dir_frame, "文档目录:", self.doc_files_dir, 2)
        self._display_directory_path(dir_frame, "提取文件:", self.extract_file_path, 3)
        # 新增：显示上传文件路径
        self._display_directory_path(dir_frame, "上传文件:", self.upload_file_path, 4)

    def _display_directory_path(self, parent_frame, label_text, path, row):
        """
        在指定框架中显示目录路径

        Args:
            parent_frame: 父框架
            label_text: 标签文本
            path: 路径
            row: 行号
        """
        ttk.Label(parent_frame, text=label_text, font=self.font).grid(row=row, column=0, sticky=tk.W, pady=5)
        ttk.Label(parent_frame, text=path, font=self.font).grid(row=row, column=1, sticky=tk.W, pady=5)

    def init_file_tab(self):
        """初始化文件管理标签页，包含文件选择、拖放区域和操作按钮"""
        # 文件管理区域
        file_frame = ttk.LabelFrame(self.file_tab, text="文件管理", padding="10")
        file_frame.pack(fill=tk.BOTH, expand=True, pady=10, padx=10)

        # 文件路径标签
        ttk.Label(file_frame, text="选择文件:", font=self.font).grid(row=0, column=0, sticky=tk.W, pady=5)

        # 文件路径输入框
        self.file_path_var = tk.StringVar()
        file_entry = ttk.Entry(file_frame, textvariable=self.file_path_var, width=60, font=self.font)
        file_entry.grid(row=0, column=1, sticky=tk.W + tk.E, pady=5, padx=5)

        # 浏览文件按钮
        browse_btn = ttk.Button(file_frame, text="浏览", command=self.browse_file)
        browse_btn.grid(row=0, column=2, sticky=tk.W, pady=5, padx=5)

        # ========== 新增：文件操作按钮 ==========
        btn_frame = ttk.Frame(file_frame)
        btn_frame.grid(row=0, column=3, sticky=tk.W, pady=5, padx=5)

        # 清空文档目录按钮
        clear_doc_btn = ttk.Button(btn_frame, text="清空文档目录", command=self.clear_doc_directory)
        clear_doc_btn.pack(side=tk.TOP, pady=2)

        # 移动图片到备份目录按钮
        move_img_btn = ttk.Button(btn_frame, text="移动图片到备份", command=self.move_images_to_backup)
        move_img_btn.pack(side=tk.TOP, pady=2)

        # 移动选择文件按钮
        move_file_btn = ttk.Button(btn_frame, text="移动选中文件", command=self.move_selected_file)
        move_file_btn.pack(side=tk.TOP, pady=2)

        # 拖放区域
        drop_frame = ttk.LabelFrame(file_frame, text="拖放文件到此处", padding="20")
        drop_frame.grid(row=1, column=0, columnspan=4, sticky=tk.NSEW, pady=10)  # 调整列数为4

        # 使框架可扩展
        file_frame.columnconfigure(1, weight=1)
        file_frame.rowconfigure(1, weight=1)

        # 拖放区域文本
        self.drop_text = tk.StringVar()
        self.drop_text.set("请拖放文件到此处，或使用上方的浏览按钮选择文件")
        drop_label = ttk.Label(drop_frame, textvariable=self.drop_text, font=self.font, justify=tk.CENTER)
        drop_label.pack(expand=True)

    def _create_action_button(self, parent_frame, text, command):
        """
        在指定框架中创建操作按钮

        Args:
            parent_frame: 父框架
            text: 按钮文本
            command: 按钮点击事件处理函数
        """
        button = ttk.Button(parent_frame, text=text, command=command)
        button.pack(side=tk.LEFT, padx=5)

    def init_action_tab(self):
        """初始化操作标签页，包含执行按钮和操作说明"""
        # 操作区域
        action_frame = ttk.LabelFrame(self.action_tab, text="执行操作", padding="20")
        action_frame.pack(fill=tk.BOTH, expand=True, pady=10, padx=10)

        # 按钮容器
        btn_container = ttk.Frame(action_frame)
        btn_container.pack(pady=20)

        # 执行解析文件按钮
        extract_btn = ttk.Button(btn_container, text="执行解析文件", command=self.execute_extract_file, width=30)
        extract_btn.pack(side=tk.LEFT, padx=10)

        # 新增：执行上传文件按钮
        upload_btn = ttk.Button(btn_container, text="执行上传文件", command=self.execute_upload_file, width=30)
        upload_btn.pack(side=tk.LEFT, padx=10)

        # 操作说明
        instructions = self._get_operation_instructions()
        instructions_text = scrolledtext.ScrolledText(action_frame, wrap=tk.WORD, width=80, height=15, font=self.font)
        instructions_text.pack(fill=tk.BOTH, expand=True, pady=10)
        instructions_text.insert(tk.END, instructions)
        instructions_text.config(state=tk.DISABLED)

    def _get_operation_instructions(self):
        """获取操作说明文本"""
        return """
        操作说明：
        1. 在"配置"标签页中，输入或加载Cookie并保存
        2. 在"文件管理"标签页中，选择要处理的文件
        3. 点击"清空文档目录"按钮清空文档目录
        4. 点击"移动图片到备份目录"按钮将图片移动到备份目录
        5. 点击"移动选择的文件到文档目录"按钮将选择的文件移动到文档目录
        6. 在本标签页中，点击"执行解析文件"按钮执行解析操作
        7. 在本标签页中，点击"执行上传文件"按钮执行上传操作
        8. 在"日志"标签页中查看操作日志
        9. 右侧面板可实时查看图片目录和文档目录的文件列表
        """

    def init_log_tab(self):
        """初始化日志标签页，包含日志显示和清空功能"""
        # 日志区域
        log_frame = ttk.LabelFrame(self.log_tab, text="操作日志", padding="10")
        log_frame.pack(fill=tk.BOTH, expand=True, pady=10, padx=10)

        # 日志文本框
        self.log_text = scrolledtext.ScrolledText(log_frame, wrap=tk.WORD, width=80, height=25, font=self.font)
        self.log_text.pack(fill=tk.BOTH, expand=True)
        self.log_text.config(state=tk.DISABLED)

        # 清空日志按钮
        clear_log_btn = ttk.Button(log_frame, text="清空日志", command=self.clear_logs)
        clear_log_btn.pack(side=tk.RIGHT, pady=5)

    # ========== 新增：初始化文件浏览面板 ==========
    def init_file_browser_panel(self, parent_frame):
        """初始化右侧文件浏览面板"""
        # 创建文件浏览主框架
        browser_frame = ttk.LabelFrame(parent_frame, text="文件目录浏览", padding="10")
        browser_frame.pack(fill=tk.BOTH, expand=True, pady=5)

        # 目录选择区域
        dir_select_frame = ttk.Frame(browser_frame)
        dir_select_frame.pack(fill=tk.X, pady=5)

        # 目录选择下拉框
        self.selected_dir = tk.StringVar(value="图片目录")
        dir_options = ["图片目录", "文档目录", "图片备份目录"]
        dir_combo = ttk.Combobox(dir_select_frame, textvariable=self.selected_dir, values=dir_options, state="readonly",
                                 width=15)
        dir_combo.pack(side=tk.LEFT, padx=5)
        dir_combo.bind("<<ComboboxSelected>>", lambda e: self.refresh_file_list())

        # 刷新按钮
        refresh_btn = ttk.Button(dir_select_frame, text="刷新", command=self.refresh_file_list)
        refresh_btn.pack(side=tk.LEFT, padx=5)

        # 打开目录按钮
        open_dir_btn = ttk.Button(dir_select_frame, text="打开目录", command=self.open_selected_directory)
        open_dir_btn.pack(side=tk.LEFT, padx=5)

        # 文件列表区域
        list_frame = ttk.Frame(browser_frame)
        list_frame.pack(fill=tk.BOTH, expand=True, pady=5)

        # 文件列表树视图
        columns = ("name", "size", "mtime")
        self.file_tree = ttk.Treeview(list_frame, columns=columns, show="headings", height=15)

        # 设置列标题
        self.file_tree.heading("name", text="文件名")
        self.file_tree.heading("size", text="大小(KB)")
        self.file_tree.heading("mtime", text="修改时间")

        # 设置列宽
        self.file_tree.column("name", width=180, anchor=tk.W)
        self.file_tree.column("size", width=60, anchor=tk.CENTER)
        self.file_tree.column("mtime", width=100, anchor=tk.CENTER)

        # 滚动条
        tree_scroll = ttk.Scrollbar(list_frame, orient=tk.VERTICAL, command=self.file_tree.yview)
        self.file_tree.configure(yscrollcommand=tree_scroll.set)

        # 布局
        self.file_tree.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        tree_scroll.pack(side=tk.RIGHT, fill=tk.Y)

        # 绑定双击事件（打开文件）
        self.file_tree.bind("<Double-1>", self.on_file_double_click)

        # 文件信息标签
        self.file_info_var = tk.StringVar(value="选中文件：无")
        info_label = ttk.Label(browser_frame, textvariable=self.file_info_var, font=self.font)
        info_label.pack(fill=tk.X, pady=5)

        # 绑定选中事件
        self.file_tree.bind("<<TreeviewSelect>>", self.on_file_select)

    # ========== 新增：文件浏览相关方法 ==========
    def get_current_directory(self):
        """获取当前选中的目录路径"""
        dir_type = self.selected_dir.get()
        if dir_type == "图片目录":
            return self.imgs_dir
        elif dir_type == "文档目录":
            return self.doc_files_dir
        elif dir_type == "图片备份目录":
            return self.imgs_old_dir
        return ""

    def refresh_file_list(self):
        """刷新文件列表"""
        # 清空现有列表
        for item in self.file_tree.get_children():
            self.file_tree.delete(item)

        # 获取当前目录
        current_dir = self.get_current_directory()
        if not os.path.exists(current_dir):
            self.add_log(f"目录不存在: {current_dir}")
            return

        # 遍历目录中的文件
        try:
            for filename in os.listdir(current_dir):
                file_path = os.path.join(current_dir, filename)
                if os.path.isfile(file_path):
                    # 获取文件信息
                    file_size = os.path.getsize(file_path) / 1024  # 转换为KB
                    mtime = os.path.getmtime(file_path)
                    mtime_str = datetime.fromtimestamp(mtime).strftime("%Y-%m-%d %H:%M")

                    # 插入到树视图
                    self.file_tree.insert("", tk.END, values=(filename, f"{file_size:.1f}", mtime_str),
                                          tags=(file_path,))

            self.add_log(f"已刷新 {self.selected_dir.get()} 文件列表，共 {len(self.file_tree.get_children())} 个文件")
        except Exception as e:
            self.add_log(f"刷新文件列表失败: {str(e)}")
            messagebox.showerror("错误", f"刷新文件列表失败: {str(e)}")

    def on_file_select(self, event):
        """文件选中事件"""
        selected_items = self.file_tree.selection()
        if selected_items:
            item = selected_items[0]
            file_path = self.file_tree.item(item, "tags")[0] if self.file_tree.item(item, "tags") else ""
            filename = self.file_tree.item(item, "values")[0]
            self.file_info_var.set(f"选中文件：{filename}")
        else:
            self.file_info_var.set("选中文件：无")

    def on_file_double_click(self, event):
        """双击打开文件"""
        selected_items = self.file_tree.selection()
        if selected_items:
            item = selected_items[0]
            file_path = self.file_tree.item(item, "tags")[0] if self.file_tree.item(item, "tags") else ""
            if file_path and os.path.exists(file_path):
                try:
                    # 根据系统打开文件
                    if platform.system() == "Windows":
                        os.startfile(file_path)
                    elif platform.system() == "Darwin":  # macOS
                        subprocess.run(["open", file_path])
                    else:  # Linux
                        subprocess.run(["xdg-open", file_path])
                    self.add_log(f"已打开文件: {file_path}")
                except Exception as e:
                    self.add_log(f"打开文件失败: {str(e)}")
                    messagebox.showerror("错误", f"打开文件失败: {str(e)}")

    def open_selected_directory(self):
        """打开选中的目录"""
        current_dir = self.get_current_directory()
        if os.path.exists(current_dir):
            try:
                if platform.system() == "Windows":
                    os.startfile(current_dir)
                elif platform.system() == "Darwin":
                    subprocess.run(["open", current_dir])
                else:
                    subprocess.run(["xdg-open", current_dir])
                self.add_log(f"已打开目录: {current_dir}")
            except Exception as e:
                self.add_log(f"打开目录失败: {str(e)}")
                messagebox.showerror("错误", f"打开目录失败: {str(e)}")
        else:
            messagebox.showwarning("警告", f"目录不存在: {current_dir}")
            self.add_log(f"警告: 目录不存在: {current_dir}")

    # ========== 原有方法保持不变 ==========
    def load_current_cookie(self):
        """从配置文件加载当前Cookie值（逐行读取，稳定可靠）"""
        try:
            if not os.path.exists(self.conf_file_path):
                messagebox.showerror("错误", f"配置文件不存在: {self.conf_file_path}")
                self.add_log(f"错误: 配置文件不存在: {self.conf_file_path}")
                return

            # 逐行读取文件，找Cookie行
            cookie_value = ""
            with open(self.conf_file_path, 'r', encoding='utf-8') as f:
                for line_num, line in enumerate(f, 1):
                    if line.strip().startswith("Cookie") and "=" in line:
                        # 拆分等号，提取引号内的内容
                        parts = line.split("=", 1)
                        if len(parts) == 2:
                            value_part = parts[1].strip()
                            # 去掉前后的引号
                            if value_part.startswith('"') and value_part.endswith('"'):
                                cookie_value = value_part[1:-1]
                                self.add_log(f"在第{line_num}行找到Cookie")
                                break

            if cookie_value:
                # 清空输入框并插入Cookie
                self.cookie_text.delete(1.0, tk.END)
                self.cookie_text.insert(1.0, cookie_value)
                self.add_log("已加载当前Cookie")
            else:
                messagebox.showwarning("警告", "未在配置文件中找到Cookie行，请检查文件格式")
                self.add_log("警告: 未在配置文件中找到Cookie行")

        except Exception as e:
            messagebox.showerror("错误", f"加载Cookie时出错: {str(e)}")
            self.add_log(f"错误: 加载Cookie时出错: {str(e)}")

    def save_cookie(self):
        """保存Cookie（核心修复：逐行替换，避开正则）"""
        try:
            # 从多行文本框获取Cookie（去首尾空格/换行）
            cookie_value = self.cookie_text.get(1.0, tk.END).strip()
            if not cookie_value:
                messagebox.showwarning("警告", "Cookie值不能为空")
                return

            if not os.path.exists(self.conf_file_path):
                messagebox.showerror("错误", f"配置文件不存在: {self.conf_file_path}")
                self.add_log(f"错误: 配置文件不存在: {self.conf_file_path}")
                return

            # 逐行读取并修改
            new_lines = []
            cookie_updated = False
            with open(self.conf_file_path, 'r', encoding='utf-8') as f:
                for line_num, line in enumerate(f, 1):
                    # 找到Cookie行
                    if line.strip().startswith("Cookie") and "=" in line:
                        # 保留原有格式（等号前的空格），只替换值
                        parts = line.split("=", 1)
                        if len(parts) == 2:
                            # 拼接新行：原有前缀 + = + 新Cookie值（带引号）
                            new_line = f'{parts[0]}= "{cookie_value}"\n'
                            new_lines.append(new_line)
                            cookie_updated = True
                            self.add_log(f"已修改第{line_num}行的Cookie值")
                        else:
                            new_lines.append(line)
                    else:
                        new_lines.append(line)

            # 检查是否更新成功
            if not cookie_updated:
                # 没找到Cookie行，尝试在const块末尾添加
                self.add_log("未找到Cookie行，尝试在const块末尾添加")
                const_found = False
                for i, line in enumerate(new_lines):
                    if line.strip().startswith("const ("):
                        const_found = True
                        # 在const块内最后一行前插入Cookie行
                        # 找const块的结束符 ")"
                        for j in range(i + 1, len(new_lines)):
                            if new_lines[j].strip() == ")":
                                # 插入Cookie行（保持格式和其他行一致）
                                new_lines.insert(j, f'\tCookie     = "{cookie_value}"\n')
                                cookie_updated = True
                                self.add_log(f"在const块中添加了Cookie行")
                                break
                        break
                if not const_found:
                    messagebox.showwarning("警告", "配置文件中未找到const块，无法添加Cookie")
                    self.add_log("警告: 配置文件中未找到const块")
                    return

            # 写入修改后的内容
            with open(self.conf_file_path, 'w', encoding='utf-8') as f:
                f.writelines(new_lines)

            if cookie_updated:
                messagebox.showinfo("成功", "Cookie已保存")
                self.add_log("Cookie保存成功")
            else:
                messagebox.showwarning("警告", "未找到Cookie行且无法添加，请手动检查配置文件格式")
                self.add_log("警告: 未找到Cookie行且无法添加")

        except Exception as e:
            messagebox.showerror("错误", f"保存Cookie时出错: {str(e)}")
            self.add_log(f"错误: 保存Cookie时出错: {str(e)}")

    def browse_file(self):
        file_path = filedialog.askopenfilename(title="选择文件")
        if file_path:
            self.file_path_var.set(file_path)
            self.drop_text.set(f"已选择文件: {os.path.basename(file_path)}")
            self.add_log(f"已选择文件: {file_path}")

    def clear_doc_directory(self):
        try:
            if not os.path.exists(self.doc_files_dir):
                os.makedirs(self.doc_files_dir)
                self.add_log(f"已创建文档目录: {self.doc_files_dir}")
                messagebox.showinfo("成功", "文档目录不存在，已创建")
                # 清空后刷新文件列表
                self.refresh_file_list()
                return

            # 清空目录（兼容文件/子目录）
            for filename in os.listdir(self.doc_files_dir):
                file_path = os.path.join(self.doc_files_dir, filename)
                try:
                    if os.path.isfile(file_path) or os.path.islink(file_path):
                        os.unlink(file_path)
                        self.add_log(f"已删除文件: {file_path}")
                    elif os.path.isdir(file_path):
                        shutil.rmtree(file_path)
                        self.add_log(f"已删除目录: {file_path}")
                except Exception as e:
                    self.add_log(f"删除文件失败 {file_path}: {str(e)}")

            messagebox.showinfo("成功", "文档目录已清空")
            self.add_log("文档目录已清空")
            # 清空后刷新文件列表
            self.refresh_file_list()

        except Exception as e:
            messagebox.showerror("错误", f"清空文档目录时出错: {str(e)}")
            self.add_log(f"错误: 清空文档目录时出错: {str(e)}")

    def move_images_to_backup(self):
        try:
            # 检查图片目录是否存在
            if not os.path.exists(self.imgs_dir):
                messagebox.showinfo("信息", "图片目录不存在，无需移动")
                self.add_log("图片目录不存在，无需移动")
                return

            # 检查备份目录是否存在，不存在则创建
            if not os.path.exists(self.imgs_old_dir):
                os.makedirs(self.imgs_old_dir)
                self.add_log(f"已创建图片备份目录: {self.imgs_old_dir}")

            # 移动图片文件
            moved_count = 0
            for filename in os.listdir(self.imgs_dir):
                src_path = os.path.join(self.imgs_dir, filename)
                # 跳过目录，只移动文件
                if os.path.isdir(src_path):
                    continue

                dst_path = os.path.join(self.imgs_old_dir, filename)

                # 如果目标文件已存在，添加时间戳
                if os.path.exists(dst_path):
                    base, ext = os.path.splitext(filename)
                    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                    dst_path = os.path.join(self.imgs_old_dir, f"{base}_{timestamp}{ext}")

                shutil.move(src_path, dst_path)
                self.add_log(f"已移动图片: {src_path} -> {dst_path}")
                moved_count += 1

            messagebox.showinfo("成功", f"已移动 {moved_count} 个图片文件到备份目录")
            self.add_log(f"已移动 {moved_count} 个图片文件到备份目录")
            # 移动后刷新文件列表
            self.refresh_file_list()

        except Exception as e:
            messagebox.showerror("错误", f"移动图片到备份目录时出错: {str(e)}")
            self.add_log(f"错误: 移动图片到备份目录时出错: {str(e)}")

    def move_selected_file(self):
        try:
            file_path = self.file_path_var.get().strip()
            if not file_path:
                messagebox.showwarning("警告", "请先选择文件")
                return

            if not os.path.exists(file_path):
                messagebox.showwarning("警告", "选择的文件不存在")
                self.add_log(f"警告: 选择的文件不存在: {file_path}")
                return

            # 检查文档目录是否存在，不存在则创建
            if not os.path.exists(self.doc_files_dir):
                os.makedirs(self.doc_files_dir)
                self.add_log(f"已创建文档目录: {self.doc_files_dir}")

            # 获取文件名
            filename = os.path.basename(file_path)
            dst_path = os.path.join(self.doc_files_dir, filename)

            # 如果目标文件已存在，添加时间戳
            if os.path.exists(dst_path):
                base, ext = os.path.splitext(filename)
                timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                dst_path = os.path.join(self.doc_files_dir, f"{base}_{timestamp}{ext}")

            # 移动文件
            shutil.move(file_path, dst_path)
            self.add_log(f"已移动文件: {file_path} -> {dst_path}")
            messagebox.showinfo("成功", f"文件已移动到文档目录: {dst_path}")

            # 清空文件路径输入框
            self.file_path_var.set("")
            self.drop_text.set("请拖放文件到此处，或使用上方的浏览按钮选择文件")

            # 移动后刷新文件列表
            self.refresh_file_list()

        except Exception as e:
            messagebox.showerror("错误", f"移动文件时出错: {str(e)}")
            self.add_log(f"错误: 移动文件时出错: {str(e)}")

    def _safe_decode(self, byte_data):
        """
        安全解码字节数据，优先utf-8，失败则用gbk，最后用replace模式忽略错误
        """
        if not byte_data:
            return ""
        try:
            # 优先尝试utf-8（Go程序常用编码）
            return byte_data.decode("utf-8")
        except UnicodeDecodeError:
            try:
                # 再尝试gbk（Windows默认编码）
                return byte_data.decode("gbk")
            except UnicodeDecodeError:
                # 最后忽略错误，替换无法解码的字符
                return byte_data.decode("utf-8", errors="replace")

    def execute_extract_file(self):
        """
        执行Go代码（核心修改：对齐GoLand的命令格式 + 修复编码错误）
        步骤：1. 构建Go程序生成可执行文件 2. 执行可执行文件 3. 清理临时文件
        """
        try:
            if not os.path.exists(self.extract_file_path):
                messagebox.showerror("错误", f"提取文件不存在: {self.extract_file_path}")
                self.add_log(f"错误: 提取文件不存在: {self.extract_file_path}")
                return

            # 1. 准备环境（完全对齐GoLand的环境变量）
            extract_dir = os.path.dirname(self.extract_file_path)
            # 临时可执行文件路径（模仿GoLand的临时目录）
            temp_exe_path = os.path.join(
                os.environ.get("TEMP", "C:\\Temp"),
                "GoLand",
                f"___go_build_extractFile_{datetime.now().strftime('%Y%m%d%H%M%S')}.exe"
            )
            # 创建临时目录（如果不存在）
            os.makedirs(os.path.dirname(temp_exe_path), exist_ok=True)

            # 2. 设置环境变量（GOROOT/GOPATH和GoLand一致）
            env = os.environ.copy()
            env["GOROOT"] = self.goroot
            env["GOPATH"] = self.gopath
            # 可选：添加Go bin到PATH，确保go命令可执行
            env["PATH"] = f"{self.goroot}\\bin;{env['PATH']}"
            # 强制设置Go程序输出编码为utf-8
            env["GODEBUG"] = "utf8=1"
            env["PYTHONIOENCODING"] = "utf-8"

            self.add_log("开始构建解析文件Go程序...")
            self.add_log(
                f"构建命令: {self.go_bin_path} build -o {temp_exe_path} -gcflags \"all=-N -l\" {self.extract_file_path}")

            # 3. 执行Go build（和GoLand完全一致的构建命令，禁用text模式，手动解码）
            build_process = subprocess.Popen(
                [
                    self.go_bin_path,
                    "build",
                    "-o", temp_exe_path,
                    "-gcflags", "all=-N -l",  # 禁用优化和内联，方便调试（和GoLand一致）
                    self.extract_file_path
                ],
                cwd=extract_dir,
                env=env,
                stdout=subprocess.PIPE,
                stderr=subprocess.PIPE,
                # 关键：不使用text=True，返回字节数据，手动解码
                creationflags=subprocess.CREATE_NO_WINDOW  # 隐藏命令行窗口
            )
            # 读取字节数据，手动安全解码
            build_stdout_bytes, build_stderr_bytes = build_process.communicate()
            build_stdout = self._safe_decode(build_stdout_bytes)
            build_stderr = self._safe_decode(build_stderr_bytes)

            # 检查构建是否成功
            if build_process.returncode != 0:
                self.add_log(f"解析文件Go程序构建失败 - 标准输出: {build_stdout}")
                self.add_log(f"解析文件Go程序构建失败 - 错误输出: {build_stderr}")
                messagebox.showerror("错误", f"解析文件Go程序构建失败:\n{build_stderr}")
                return
            self.add_log("解析文件Go程序构建成功！")

            # 4. 执行构建好的可执行文件（模仿GoLand的dlv执行逻辑，直接运行exe）
            self.add_log(f"开始执行解析文件Go可执行文件: {temp_exe_path}")

            # 创建进度窗口
            progress_window = tk.Toplevel(self.root)
            progress_window.title("执行中")
            progress_window.geometry("400x100")
            progress_window.transient(self.root)
            progress_window.grab_set()
            ttk.Label(progress_window, text="正在执行解析文件，请稍候...", font=self.font).pack(pady=20)
            progress_window.update()

            # 执行可执行文件（同样禁用text模式，手动解码）
            run_process = subprocess.Popen(
                [temp_exe_path],
                cwd=extract_dir,
                env=env,
                stdout=subprocess.PIPE,
                stderr=subprocess.PIPE,
                creationflags=subprocess.CREATE_NO_WINDOW
            )
            # 读取字节数据，手动安全解码
            run_stdout_bytes, run_stderr_bytes = run_process.communicate()
            run_stdout = self._safe_decode(run_stdout_bytes)
            run_stderr = self._safe_decode(run_stderr_bytes)

            # 关闭进度窗口
            progress_window.destroy()

            # 5. 检查执行结果
            if run_process.returncode == 0:
                self.add_log(f"解析文件Go程序执行成功 - 输出: {run_stdout}")
                messagebox.showinfo("成功", f"解析文件执行成功！\n输出: {run_stdout}")
                # 执行成功后刷新文件列表（可能生成了新文件）
                self.refresh_file_list()
            else:
                self.add_log(f"解析文件Go程序执行失败 - 输出: {run_stdout}")
                self.add_log(f"解析文件Go程序执行失败 - 错误: {run_stderr}")
                messagebox.showerror("错误", f"解析文件执行失败:\n{run_stderr}")

            # 6. 清理临时可执行文件（可选，保留日志方便排查）
            try:
                if os.path.exists(temp_exe_path):
                    os.remove(temp_exe_path)
                    self.add_log("已清理解析文件临时可执行文件")
            except Exception as e:
                self.add_log(f"清理解析文件临时文件失败: {str(e)}")

        except Exception as e:
            messagebox.showerror("错误", f"执行解析文件Go代码时出错: {str(e)}")
            self.add_log(f"错误: 执行解析文件Go代码时出错: {str(e)}")

    # ========== 新增：执行上传文件方法 ==========
    def execute_upload_file(self):
        """
        执行上传文件Go代码，逻辑和执行解析文件一致，仅修改文件路径和相关日志
        步骤：1. 构建Go程序生成可执行文件 2. 执行可执行文件 3. 清理临时文件
        """
        try:
            if not os.path.exists(self.upload_file_path):
                messagebox.showerror("错误", f"上传文件不存在: {self.upload_file_path}")
                self.add_log(f"错误: 上传文件不存在: {self.upload_file_path}")
                return

            # 1. 准备环境（完全对齐GoLand的环境变量）
            upload_dir = os.path.dirname(self.upload_file_path)
            # 临时可执行文件路径（模仿GoLand的临时目录，区分解析文件）
            temp_exe_path = os.path.join(
                os.environ.get("TEMP", "C:\\Temp"),
                "GoLand",
                f"___go_build_uploadFile_{datetime.now().strftime('%Y%m%d%H%M%S')}.exe"
            )
            # 创建临时目录（如果不存在）
            os.makedirs(os.path.dirname(temp_exe_path), exist_ok=True)

            # 2. 设置环境变量（GOROOT/GOPATH和GoLand一致）
            env = os.environ.copy()
            env["GOROOT"] = self.goroot
            env["GOPATH"] = self.gopath
            # 可选：添加Go bin到PATH，确保go命令可执行
            env["PATH"] = f"{self.goroot}\\bin;{env['PATH']}"
            # 强制设置Go程序输出编码为utf-8
            env["GODEBUG"] = "utf8=1"
            env["PYTHONIOENCODING"] = "utf-8"

            self.add_log("开始构建上传文件Go程序...")
            self.add_log(
                f"构建命令: {self.go_bin_path} build -o {temp_exe_path} -gcflags \"all=-N -l\" {self.upload_file_path}")

            # 3. 执行Go build（和GoLand完全一致的构建命令，禁用text模式，手动解码）
            build_process = subprocess.Popen(
                [
                    self.go_bin_path,
                    "build",
                    "-o", temp_exe_path,
                    "-gcflags", "all=-N -l",  # 禁用优化和内联，方便调试（和GoLand一致）
                    self.upload_file_path
                ],
                cwd=upload_dir,
                env=env,
                stdout=subprocess.PIPE,
                stderr=subprocess.PIPE,
                # 关键：不使用text=True，返回字节数据，手动解码
                creationflags=subprocess.CREATE_NO_WINDOW  # 隐藏命令行窗口
            )
            # 读取字节数据，手动安全解码
            build_stdout_bytes, build_stderr_bytes = build_process.communicate()
            build_stdout = self._safe_decode(build_stdout_bytes)
            build_stderr = self._safe_decode(build_stderr_bytes)

            # 检查构建是否成功
            if build_process.returncode != 0:
                self.add_log(f"上传文件Go程序构建失败 - 标准输出: {build_stdout}")
                self.add_log(f"上传文件Go程序构建失败 - 错误输出: {build_stderr}")
                messagebox.showerror("错误", f"上传文件Go程序构建失败:\n{build_stderr}")
                return
            self.add_log("上传文件Go程序构建成功！")

            # 4. 执行构建好的可执行文件（模仿GoLand的dlv执行逻辑，直接运行exe）
            self.add_log(f"开始执行上传文件Go可执行文件: {temp_exe_path}")

            # 创建进度窗口
            progress_window = tk.Toplevel(self.root)
            progress_window.title("执行中")
            progress_window.geometry("400x100")
            progress_window.transient(self.root)
            progress_window.grab_set()
            ttk.Label(progress_window, text="正在执行上传文件，请稍候...", font=self.font).pack(pady=20)
            progress_window.update()

            # 执行可执行文件（同样禁用text模式，手动解码）
            run_process = subprocess.Popen(
                [temp_exe_path],
                cwd=upload_dir,
                env=env,
                stdout=subprocess.PIPE,
                stderr=subprocess.PIPE,
                creationflags=subprocess.CREATE_NO_WINDOW
            )
            # 读取字节数据，手动安全解码
            run_stdout_bytes, run_stderr_bytes = run_process.communicate()
            run_stdout = self._safe_decode(run_stdout_bytes)
            run_stderr = self._safe_decode(run_stderr_bytes)

            # 关闭进度窗口
            progress_window.destroy()

            # 5. 检查执行结果
            if run_process.returncode == 0:
                self.add_log(f"上传文件Go程序执行成功 - 输出: {run_stdout}")
                messagebox.showinfo("成功", f"上传文件执行成功！\n输出: {run_stdout}")
                # 执行成功后刷新文件列表（可能生成了新文件）
                self.refresh_file_list()
            else:
                self.add_log(f"上传文件Go程序执行失败 - 输出: {run_stdout}")
                self.add_log(f"上传文件Go程序执行失败 - 错误: {run_stderr}")
                messagebox.showerror("错误", f"上传文件执行失败:\n{run_stderr}")

            # 6. 清理临时可执行文件（可选，保留日志方便排查）
            try:
                if os.path.exists(temp_exe_path):
                    os.remove(temp_exe_path)
                    self.add_log("已清理上传文件临时可执行文件")
            except Exception as e:
                self.add_log(f"清理上传文件临时文件失败: {str(e)}")

        except Exception as e:
            messagebox.showerror("错误", f"执行上传文件Go代码时出错: {str(e)}")
            self.add_log(f"错误: 执行上传文件Go代码时出错: {str(e)}")

    def add_log(self, message):
        timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        log_entry = f"[{timestamp}] {message}"
        self.logs.append(log_entry)

        # 更新日志文本框
        self.log_text.config(state=tk.NORMAL)
        self.log_text.insert(tk.END, log_entry + "\n")
        self.log_text.see(tk.END)
        self.log_text.config(state=tk.DISABLED)

    def clear_logs(self):
        self.logs = []
        self.log_text.config(state=tk.NORMAL)
        self.log_text.delete(1.0, tk.END)
        self.log_text.config(state=tk.DISABLED)
        self.add_log("日志已清空")

    def check_directories(self):
        # 检查配置文件是否存在
        if not os.path.exists(self.conf_file_path):
            self.add_log(f"警告: 配置文件不存在: {self.conf_file_path}")

        # 检查图片目录是否存在
        if not os.path.exists(self.imgs_dir):
            self.add_log(f"信息: 图片目录不存在: {self.imgs_dir}")

        # 检查备份目录是否存在
        if not os.path.exists(self.imgs_old_dir):
            self.add_log(f"信息: 图片备份目录不存在: {self.imgs_old_dir}")

        # 检查文档目录是否存在
        if not os.path.exists(self.doc_files_dir):
            self.add_log(f"信息: 文档目录不存在: {self.doc_files_dir}")

        # 检查提取文件是否存在
        if not os.path.exists(self.extract_file_path):
            self.add_log(f"警告: 提取文件不存在: {self.extract_file_path}")

        # 新增：检查上传文件是否存在
        if not os.path.exists(self.upload_file_path):
            self.add_log(f"警告: 上传文件不存在: {self.upload_file_path}")

        # 检查Go环境是否有效
        if not os.path.exists(self.go_bin_path):
            self.add_log(f"警告: Go可执行文件不存在: {self.go_bin_path}")
        else:
            self.add_log("Go环境检测正常")


if __name__ == "__main__":
    root = tk.Tk()
    app = FileUploadTool(root_window=root)
    root.mainloop()